...
object FrameType {
     ...
     * (see `com.dresos.nexus.mesh.PendingInbound`): the locally-delivered types only. PROFILE and KEY_REQ
...
/**
 * Whether this frame is carried for store-and-forward delivery (see `com.dresos.nexus.mesh.ForwardSync`).
 ...
 * (content hash of the avatar blob), [pubKey] (base64 [com.dresos.nexus.mesh.crypto.PublicKeyBundle];
 ...
 * and [com.dresos.nexus.mesh.crypto.MessageContent] (encrypted DM/group). A plain nested value.
 */
@Serializable
data class ReplyRef(
    val messageId: String,
 ...
 * (see [com.dresos.nexus.data.message.Conversations.groupIdFor]); [name] is set ONLY by an explicit
 ...
 * content key encrypts the [com.dresos.nexus.mesh.crypto.MessageContent] with AES-256-GCM into [ct]
 ...
 * to interoperate (the [com.dresos.nexus.mesh.wifiaware.WifiAwareTransport] service name is bumped in lockstep).
 */
object WireCodec {
    @PublishedApi
