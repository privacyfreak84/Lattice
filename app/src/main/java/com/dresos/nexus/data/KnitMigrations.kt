...
/**
 * The registry of tested schema migrations applied in [NexusDatabase.build].
 *
 ...
 * `NexusDatabaseMigrationTest`) instead of silently wiping user data. So this is the single place production
 ...
 * the driver-based `migrate(SQLiteConnection)` override (matching the `NexusDatabaseMigrationTest` harness), e.g.:
 *
 * ```
