 ...
 * [com.dresos.nexus.data.blob.BlobEntity]) and are only set once the avatar payload has arrived, so a
 * non-null hash always implies the blob is present.
 *
 * [pubKey] is the peer's pinned end-to-end public-key bundle (base64; see
 * [com.dresos.nexus.mesh.crypto.PublicKeyBundle]), learned from their profile frame on a
 ...
 *
 * [deviceTag] is the peer's key-independent device tag (see [com.dresos.nexus.identity.DeviceTag]),
 ...
 * [com.dresos.nexus.mesh.protocol.Protocol]), learned (authenticated) from their profile frame; null
