...
object FrameType {
     ...
     * (see `org.lattice.mesh.PendingInbound`): the locally-delivered types only. PROFILE and KEY_REQ
...
/**
 * Whether this frame is carried for store-and-forward delivery (see `org.lattice.mesh.ForwardSync`).
 ...
 * (content hash of the avatar blob), [pubKey] (base64 [org.lattice.mesh.crypto.PublicKeyBundle];
 ...
 * and [org.lattice.mesh.crypto.MessageContent] (encrypted DM/group). A plain nested value.
 */
@Serializable
data class ReplyRef(
    val messageId: String,
 ...
 * (see [org.lattice.data.message.Conversations.groupIdFor]); [name] is set ONLY by an explicit
 ...
 * content key encrypts the [org.lattice.mesh.crypto.MessageContent] with AES-256-GCM into [ct]
 ...
 * to interoperate (the [org.lattice.mesh.wifiaware.WifiAwareTransport] service name is bumped in lockstep).
 */
object WireCodec {
    @PublishedApi
