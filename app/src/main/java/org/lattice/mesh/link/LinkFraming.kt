 ...
 * ([org.lattice.mesh.link.LinkSocket]) share it unchanged.
 *
 * Each record is `[type:1][len:4 big-endian][payload:len]`. [Type.FRAME] carries one CBOR
 * [org.lattice.mesh.protocol.WireEnvelope]; a file is streamed as [Type.FILE_HEADER] (a
 ...
 *
 * Pure (no Android), so the codec is JVM-unit-testable ([org.lattice.LinkFramingTest]).
 */
internal object LinkFraming {
    ...
    enum class Type(
         ...
         * ([org.lattice.mesh.protocol.Protocol.advertise] bytes), so an accept-any responder — which
     ...
     * between-chunk interleave/pace points (see [org.lattice.mesh.link.FramedLink]) are frequent — the
