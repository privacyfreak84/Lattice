package org.lattice.data.settings
 ...
 * no longer persisted here — it is derived from the E2E keypair; see [org.lattice.identity.Identity].)
 */
class SettingsStore(
    ...
    /**
     * Device tags (see [org.lattice.identity.DeviceTag]) the user has blocked. Because a nodeId is
     ...
     * the peer's node id, or a group keyed by its "g-…" id (see [org.lattice.data.message.Conversations]).
     ...
     * [org.lattice.security.AppLockManager] and [org.lattice.ui.applock.AppLockGate]).
     ...
     * notification, and back to true whenever the service (re)starts, so [org.lattice.mesh.BootReceiver]
     ...
     * review-prompt engagement window (see [org.lattice.review.ReviewPromptPolicy]). Deliberately a
