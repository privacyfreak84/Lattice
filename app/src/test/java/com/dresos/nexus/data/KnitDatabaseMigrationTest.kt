 ...
 * rebuilds the DB from the checked-in `app/schemas/com.dresos.nexus.data.NexusDatabase/<version>.json`, proving
...
class NexusDatabaseMigrationTest {
    ...
    val helper =
        MigrationTestHelper(
            ...
            databaseClass = NexusDatabase::class,
        )

    @Test
    fun `the current schema (v1) creates and opens from the exported JSON`() {
        val version = 1 // NexusDatabase @Database(version = 1) — bump alongside the DB (its retention is CLASS,
    ...
    // Template for the first post-v1 migration — uncomment and fill in once NexusDatabase bumps to v2 and
